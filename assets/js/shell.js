...
(() => {
  ...
  const titles = {
    ...
    'object-wrong': { en: 'Object Wrong Machine — THE PAN Browser Tools', ja: 'Object Wrong Machine — 物体を間違わせる' },
    'panda-dub': { en: 'PANDA DUB — THE PAN Browser Tools', ja: 'PANDA DUB — ブラウザ・ダブ遊び場' },
  ...
  const JA = {
    ...
    'FREE TO USE':'無料で使えます',
    'LOCAL FIRST':'ローカル処理優先',
    'BROWSER-BASED IMAGE DESTRUCTION APPARATUS':'ブラウザだけで使える画像破壊マシン',
    'CREATE.':'つくる。',
    'PLAY.':'あそぶ。',
    'DISTORT.':'ゆがめる。',
    'Load an image. Break the signal. Export the result. Nothing is uploaded—every pixel stays in your browser.':'画像を読み込み、信号を壊して、結果を書き出す。画像はアップロードされず、すべてブラウザ内で処理されます。',
    'LOAD IMAGE':'画像を読み込む',
    ...
    'PRIVATE BY DESIGN / NO SERVER UPLOAD':'プライベート設計 / サーバー送信なし',
    'ORIGINAL':'オリジナル',
    'CURRENT':'現在',
    'COMPARE':'比較',
    ...
    'THRESHOLD':'しきい値',
    'RESET PRINT':'印刷をリセット',
    'EXPANSION BANK / RESERVED':'拡張エリア / 予約',
    'NO MODULES INSTALLED':'追加モジュールなし',
    'Images are processed locally with Canvas API. No image, filename, or personal data is transmitted or added to analytics.':'画像はCanvas APIで端末内処理されます。画像・ファイル名・個人情報は送信されず、アクセス解析にも追加されません。',
    'SURPRISE ME':'おまかせ破壊',
    ...
    'BROWSER CASSETTE DAMAGE APPARATUS':'ブラウザ・カセット破壊マシン',
    'DESTROY YOUR':'きれいな音を',
    'CLEAN AUDIO.':'壊そう。',
    'LOAD A SOUND. DAMAGE THE TAPE. KEEP THE ACCIDENT.':'音を読み込み、テープを傷めて、事故を残す。',
    'LOAD A SOUND':'音を読み込む',
    ...
    'PRIVATE SIGNAL / NOTHING IS UPLOADED':'ローカル信号 / アップロードなし',
    'PLAY':'再生',
    'PAUSE':'一時停止',
    'STOP':'停止',
    'DAMAGED':'加工後',
    ...
    'BIT DEPTH':'ビット深度',
    'SAMPLE REDUCTION':'サンプル削減',
    'RESET DAMAGE':'破損をリセット',
    'TONE':'音色',
    'LOW PASS':'ローパス',
    'RESET TONE':'音色をリセット',
    ...
    'BREAK THE':'映像を',
    'CAMERA.':'壊そう。',
    'Distort the signal. Feed back the evidence. Record the transmission.':'信号を歪め、残像を返し、映像として記録する。',
    ...
    'CHOOSE A TRANSMISSION.':'入力を選んで。',
    'THEN BREAK THE SIGNAL.':'それから信号を壊す。',
    ...
    'NORMAL':'通常',
    'DIFFERENCE':'差の絶対値',
    'PARTICLE BODY / 2.5D':'人体パーティクル / 2.5D',
    'BODY SIGNAL':'人体信号',
    'AMOUNT':'量',
    'SPREAD':'広がり',
    'DISSOLVE':'崩壊',
    ...
    'Frames are drawn into local Canvas. There is no upload endpoint and no cloud copy.':'フレームは端末内Canvasへ描画されます。アップロード先もクラウドコピーもありません。',
    'RECORD':'録画',
    'MP4 FIRST / WEBM FALLBACK':'MP4優先 / WebM予備',
    'CHECKING LOCAL RECORDER…':'端末内録画を確認中…',
    'LOCAL RECORDING COMPLETE':'端末内録画完了',
    'THE EVIDENCE.':'記録を書き出す。',
    ...
    'PHOTO / VIDEO → WRONG OBJECT / V1.1':'写真 / 動画 → 間違った物体 / V1.1',
    'MAKE THE':'物体を',
    'OBJECT WRONG.':'間違わせる。',
    'Drop a photo or video. The machine gives it fake depth, tilts it through space and chews it into pixels, dots, blocks and wire. Everything stays in your browser.':'写真や動画を入れると、偽物の奥行きを与え、空間で傾け、ピクセル・粒・ブロック・ワイヤーへ噛み砕きます。処理はすべてブラウザ内です。',
    ...
    'TOUCH THE CREATURES.':'生き物を触る。',
    'DROP THE SIGNAL.':'信号を落とす。',
    ...
    'ECHO ORBIT':'エコー軌道',
    'FEEDBACK / SAFE LIMIT':'フィードバック / 安全上限',
    'ALL SOUND STAYS IN THIS BROWSER. NO UPLOAD. NO LOGIN. JUST SIGNAL.':'音はすべてこのブラウザ内。アップロードなし。ログインなし。信号だけ。',
    'SIGNAL SLEEPING':'信号は休眠中',
    'RESET / SIGNAL SLEEPING':'リセット / 信号は休眠中',
    ...
    'TOOLS.':'TOOLS.',
    'Small, free creative machines that run where you are. No account. No installation. No cloud processing.':'その場で動く、小さくて無料のクリエイティブ・マシン。アカウント不要、インストール不要、クラウド処理なし。',
    '● AVAILABLE':'● 使用可能',
    '● AVAILABLE / V0':'● 使用可能 / V0',
    '○ PLANNED':'○ 計画中',
    'OPEN MACHINE →':'マシンを開く →',
    'BREAK AN OBJECT →':'物体を壊す →',
    'ENTER PLAYGROUND →':'遊び場へ →',
    'WHY THESE MACHINES EXIST':'なぜこのマシンを作るのか',
    'THE PAN Browser Tools is a growing shelf of interesting, free creative software made by 7thleaf Records.':'THE PAN Browser Toolsは、7thleaf Recordsが作る、無料でちょっと変なクリエイティブ・ソフトの棚です。',
    'OPEN THE DOOR':'扉を開く',
    'No login. No installation. No subscription. Open a tool in a modern browser and start making something.':'ログイン不要、インストール不要、サブスク不要。新しいブラウザでツールを開けば、すぐ作り始められます。',
    'Image Machine, Tape Machine, Signal Visualizer, Object Wrong, and Panda Dub are available now. Print and zine experiments remain future directions—not promises with artificial dates.':'Image Machine、Tape Machine、Signal Visualizer、Object Wrong、Panda Dubは公開中。PrintとZineは今後の実験候補で、無理な公開日だけ先に約束はしません。',
    ...
    'Optional anonymous analytics help us understand basic tool usage. Analytics require consent, advertising storage always remains denied, and declining never limits a tool.':'任意の匿名アクセス解析で基本的な利用状況だけを確認します。解析は同意制で、広告用ストレージは常に無効。同意しなくても機能制限はありません。',
    'THE LABEL':'レーベル',
    '7thleaf Records makes music and small creative machines. The tools are here to be useful, playful, and a little strange—not to turn every visit into a sales pitch.':'7thleaf Recordsは音楽と小さなクリエイティブ・マシンを作っています。役に立ち、遊べて、少し変であることが目的です。毎回何かを売りつけるための場所ではありません。'
  };
  ...
  function buildShelf() {
    ...
    const machines = [
      ...
      ['object-wrong','04','OBJECT WRONG','物体を間違わせる'],
